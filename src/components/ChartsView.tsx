...
export function ChartsView({ properties, selectedProperty }: ChartsViewProps) {
  ...
  const floorRevenueData =
    currentProperty?.floors.map((floor) => {
      ...
      return {
        floor: floor.number === 0 ? "Ground Floor" : `Floor ${floor.number}`,
        collected,
        potential,
  ...
  const occupancyRate =
    allUnits.length > 0 ? (allUnits.filter((unit) => unit.status !== "Vacant").length / allUnits.length) * 100 : 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{allUnits.filter((u) => u.status === "Paid").length}</div>
